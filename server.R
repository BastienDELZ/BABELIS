...
function(input, output, session) {
  ...
  output$hono_patien <- renderHighchart({
    highchart() %>%
      hc_add_series(data_comp_dep(), "line", hcaes(annee, hono_sans_depassement_moyens), yAxis=1) %>% 
      hc_add_series(data_comp_dep(), "line", hcaes(x = annee, y = nombre_patients_uniques)) %>%
      hc_yAxis_multiples(
        list(
